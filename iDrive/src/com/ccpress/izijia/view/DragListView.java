...
public class DragListView extends ListView {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (ev.getAction() == MotionEvent.ACTION_DOWN) {
			...
			View dragger = itemView.findViewById(R.id.iv_yidong);
	...
	public void onDrop(int y) {
		...
		if (dragPosition >= 0 && dragPosition < getAdapter().getCount()) {
			...
			BespokeEditAdapter adapter = (BespokeEditAdapter) getAdapter();

			BespokeVo dragSrcItem = adapter.getItem(dragSrcPosition);
			BespokeVo dragitem = adapter.getItem(dragPosition);

			//adapter.replace(dragitem, dragSrcPosition);
			//adapter.replace(dragSrcItem, dragPosition);
			//Toast.makeText(getContext(),"dragSrcPosition=="+String.valueOf(dragSrcPosition),Toast.LENGTH_LONG).show();
			//Toast.makeText(getContext(),"dragPosition=="+String.valueOf(dragPosition),Toast.LENGTH_LONG).show();
			int k=0;
			int m=adapter.getCount()-1;
			for (int i = 0; i < adapter.getCount(); i++){
				if (dragSrcPosition<dragPosition){
					if(k>=dragSrcPosition && k<dragPosition){
						adapter.replace(adapter.getItem(i+1), k);
					}
					if(k==dragPosition){
						adapter.replace(dragSrcItem,dragPosition);
					}
					...
					if(m>dragPosition && m<=dragSrcPosition){
						adapter.replace(adapter.getItem(m-1),m);
					}
					if(m==dragPosition){
						adapter.replace(dragSrcItem,dragPosition);
					}
			...
			/*for (int i = 0; i < adapter.getCount(); i++){
				...
				if (i<dragSrcPosition){

				}
				if( Math.abs(dragSrcPosition-dragPosition)==1){
				}
			}*/
				...
				PostRequest req = new PostRequest(getContext(), params,
						PersonalCenterUtils.buildUrl(getContext(), Const.CRETE_ROUTE),
						new RespListener(getContext()) {
							@Override
